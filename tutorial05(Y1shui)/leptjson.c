...
static int lept_parse_array(lept_context* c, lept_value* v) {
    ...
    EXPECT(c, '[');
    //比如 [ abc , 123 ],"["后就有空格了
    lept_parse_whitespace(c);//这里的c本来就是指针了，不用再取地址
    //by the way ，指针的地址就是指针的地址，存这个指针在内存里放哪
    ...
    for (;;) {
        ...
        // 但直接算的话就没法逐个手动free，经crt工具测试会内存泄漏，手动循环更优
...
void free_lept_value(lept_value* ptr) {
    if (ptr == NULL) {
        return;
    }
    free(ptr);
}
// 不需要再写一个用来递归的函数，直接调用lept_free就可以了，刚好也可以处理字符串
