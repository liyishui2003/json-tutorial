...
//单元测试boolean时一直通不过测试，最后修改了宏定义
//C++的enum是默认从0开始依次赋值的话，按照enum的顺序，LEPT_FALSE是有值的，和0对应的反而是LEPT_NULL
//所以把"(actual)==0"改成了"(actual)==LEPT_FALSE"后就对了
#define EXPECT_TRUE(actual) EXPECT_EQ_BASE((actual) == LEPT_TRUE, "true", "false", "%s")
#define EXPECT_FALSE(actual) EXPECT_EQ_BASE((actual) == LEPT_FALSE, "false", "true", "%s")
